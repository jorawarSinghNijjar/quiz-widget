...
import { addCurrentSuplements, addDislikedSupplements } from "../../slices/userProfile";
...
export default function Q7(props) {
  ...
  const onFormSubmit = (e) => {
    ...
    dispatch(addDislikedSupplements(selectedSupplements.map(item => item.text)));
  ...
  const onSupplementSelect = (id) => {
    ...
    if(findIdx > -1){
      ...
      setSelectedSupplements(selectedSupplements.filter((item) => {
      ...
      setSelectedSupplements(selectedSupplements.concat(supplementsArr[id-1]));
    }
